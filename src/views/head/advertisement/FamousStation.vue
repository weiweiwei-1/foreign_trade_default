...
export default {
  ...
  setup() {
    ...
    const showContent = (mark) => {
      ...
      switch (mark) {
        case 'introduction':
          content.value = '网页开发目的：\n该网页是一个测试demo，仅用于测试，不对外发布。测试包括技术测试和业务测试。\n' +
              '<p style="color: red">\测试中切勿输入个人真实信息。\n\</p>' +
              '1. 关于技术测试，测试该网页在低并发场景下的性能以及共功能可用性。\n' +
              '2. 关于业务测试，测试海外货物运费报价在外贸和货代之间的需求可行性。\n\n' +
              '业务背景：\n' +
              '海外货物运输需要尽可能节省运费成本，外贸员可以从货代处获取低报价节省运输成本，货代需要获取更多的外贸客户，增加成交率。\n\n' +
              '面向群体：货代和外贸。\n\n' +
              '业务目标：连接货代和外贸，对于海外货物的运费报价，为货代和外贸提供一个双向选择的平台。'
          key.value = 0
          break
        case 'service':
          content.value = '<p style="color: dodgerblue">\货代报价页面：www.weichat.online，业务需求：\n\</p>' +
              ...
              '<p style="color: dodgerblue">\运费询价页面：www.weichat.online/fts，业务需求:\n\</p>' +
              ...
              '1. 关于消息发送，只有进行过报价，或者是好友关系，才能相互之间发消息。\n' +
              '2. 关于好友，货代间不允许相互添加好友，外贸员之间亦不允许相互添加好友，好友关系只有外贸和货代。\n' +
              '3. 关于账号，货代和外贸不能使用同一个账号，不允许出现一个账号登录两个页面的场景，另由于阿里云禁用了主要的邮箱端口，因此无法实现邮箱验证，故采用了随机邮箱。\n' +
              '4. 关于建议反馈，点击反馈模块，扫码或者微信号添加即可反馈。\n'
          key.value = 1
          break
        case 'technique':
          content.value = '后台：Java\n' +
              ...
              '备注：作为测试demo，软件设计上使用了单机的架构设计，即使硬件支持，软件上也不支持高并发，当并发量达到一定时，会有较大概率导致数据脏读脏写\n\n' +
              '<p style="color: dodgerblue">\关于业务和技术：业务推动了技术的发展，技术让业务更有竞争力;技术是工具，为产品和业务服务\n\</p>'
          key.value = 2
          break
        case 'member':
