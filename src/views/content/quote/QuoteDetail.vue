<template>
  <div>
    <transition>
      <div id="quote-detail" v-show="quoteShow()">
        <div id="img-block">
          <img :src="$productImageUrl + productInfo.photo" alt="产品图">
        </div>
        <ul>
          <li><span>产品名</span>：<span class="value">{{ productInfo.productName }}</span></li>
          <li><span>运输路径</span>：<span
              class="value">{{ productInfo.origin }}->{{ productInfo.destinationCountry }}</span></li>
          ...
          <li class="remark"><span class="remark-key">备注</span>：<span
              class="remark-content">{{ productInfo.remark }}</span></li>
        </ul>
        <div id="quote-info">
          <div>我的报价</div>
